...
BOT_TOKEN = "5930566552:AAEZ2IIiZoWj-C-YxHiAJIeOuQMaIERSrhk"
...
commands = ["start", "/start", "go", "price", "prices", "."]


@bot.message_handler(func=msg)
def reply(message):
    words = message.text.split()
    if words[0].lower() in commands:
        newPrices = AssignDataToFirebase()
        return bot.reply_to(message, newPrices)
    else:
        return bot.reply_to(message, "Not of My Commands")
